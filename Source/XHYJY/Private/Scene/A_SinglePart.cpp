...
{
	AVDPawn* MyPawn = nullptr;
	MyPawn = Cast<AVDPawn>(UGameplayStatics::GetPlayerPawn(this, 0));
	if(MyPawn->SceneManager->GetSelectable())
	{
		Super::NotifyActorOnClicked(ButtonPressed);
		OnRocketClick.Broadcast(this);
		MyPawn->SceneManager->SetSelectable(false);
	}
}
...
void AA_SinglePart::Tick(float DeltaTime)
{
	...
	if(bAbleRotator)
	{
		FRotator NewRotator = FRotator(GetActorRotation().Pitch + 3, GetActorRotation().Yaw, GetActorRotation().Roll);
	...
	if(bAbleUp)
	{
		FVector NewLocation = FVector(GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z + 12);
